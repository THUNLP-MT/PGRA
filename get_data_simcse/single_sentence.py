...
train_json=[]
for idx in tqdm(range(len(allsents)), desc="Training retrieving"):
    single_json={'question': allsents[idx], 'answers': alllabels[idx], 'ctxs': []}
    text = allsents[idx]
    _, ret_idx = sentRetriever.faiss_retrieval(text, num_samples=150) 
    for idx_ in ret_idx[0]:
        single_json['ctxs'].append({'id':int(idx_), 'text':sentRetriever.sent_set[idx_]})
...
for idx in range(len(test_allsents)):
