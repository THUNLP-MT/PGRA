    ...
    def __getitem__(self, index):
        ...
        if 'ctxs' in example and self.n_context is not None:
            ...
            if self.tasks == "trec":
                ...
                passages = [f.format(self.idx2label(c['answers']), c['text']) + ' .' for c in contexts]
            elif self.tasks == "RTE" or self.tasks == "QNLI" or self.tasks == "MRPC":
                f = "{}" + ' ? {} , ' + " {}"
                contexts = example['ctxs'][:self.n_context]
                passages = [f.format(c['text1'], self.idx2label(c['answers']), c['text2']) + ' .' for c in contexts]
            else:
                # f = self.passage_prefix + " {}" + self.context_suffix + " {}"
                f = "{}" + self.context_suffix + " {}"
                contexts = example['ctxs'][:self.n_context]
                passages = [f.format(c['text'], self.idx2label(c['answers'])) + ' .' for c in contexts]
